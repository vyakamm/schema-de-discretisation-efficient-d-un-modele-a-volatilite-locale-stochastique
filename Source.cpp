...
int main()
{
    ...
    // graine aléatoire
     ...
     vector<Pair> x(100),x1(100),x2(100);
     Pair y,y1,y2;
 /*cout << p.next_step(1, init_spot_variance, ssv).first;
     for(size_t i=0;i<100;i++)
         x[i] = p.next_step(1, init_spot_variance, ssv);
     y= p.next_step(1, init_spot_variance, ssv);
     for (size_t i = 0; i < 100; i++)
         x1[i] = p.next_step(2, y, x);
     y1= p.next_step(2, y, x);
     for (size_t i = 0; i < 100; i++)
         x2[i] = p.next_step(3, y1, x1);
     y2 = p.next_step(3, y1, x1);
     cout << p.next_step(3, y2, x2).first;
   //  cout << p.next_step(2, y1, x1).first;*/
      cout<<p.path_maturity().first;
}