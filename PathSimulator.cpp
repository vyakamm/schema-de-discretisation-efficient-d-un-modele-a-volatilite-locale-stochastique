...
Pair PathSimulatorEuler::next_step(const size_t& time_idx, const Pair& spot_variance, vector<Pair>& spot_variance_sample) const
{
	...
	else
	{
	    ...
	    next1 = spot_variance.first + model->risk_free_rate() * delta_t * spot_variance.first + sqrt(model->local_volatility(_time_points[time_idx - 1],spot_variance_sample,spot_variance.first)) * spot_variance.first * model->psi_function(spot_variance.second) * sqrt(delta_t) * z1;
...
Pair PathSimulatorQE::next_step(const size_t& time_idx, const Pair& spot_variance, vector<Pair>& spot_variance_sample) const
{
	...
	for (size_t i = 0; i < spot_variance_sample.size(); i++)
		spot_variance_sample[i].second=pow(model->psi_function(spot_variance_sample[i].second), 2);
	//vector<Pair>v(model->get_bin()->number_of_simulations());
	//v = spot_variance_sample;
	//cout << model->local_volatility(_time_points[time_idx - 1], v, spot_variance.first)<<"\n";
	...
	double next_log_spot = current_log_spot + model->risk_free_rate() - 0.5 * model->local_volatility(_time_points[time_idx - 1], spot_variance_sample, spot_variance.first) * spot_variance.second * delta_t + (model->correlation() / model->vol_of_vol()) * r1 + rho1 * r2 * z3;
	double next1 = exp(next_log_spot);
	return Pair(next1, next2);
