...
Pair PathSimulator::path_maturity()
{
	...
	{
		v = samples;
		// samples for the computation of the conditional expectation for the next step
...
double PathSimulator::index_maturity() const
{
...
Pair PathSimulatorEuler::next_step(const size_t& time_idx, const Pair& spot_variance, const vector<Pair>& spot_variance_sample) const
{
	...
	else
	{
		...
		if (time_idx == 1)
			next1 = spot_variance.first + model->risk_free_rate() * delta_t * spot_variance.first + sqrt(pow((model->get_dupire_vol()).local_volatility(_time_points[time_idx - 1], spot_variance.first), 2) / model->psi_function(spot_variance.second)) * spot_variance.first * model->psi_function(spot_variance.second) * sqrt(delta_t) * z1;
		else
		{
			// Applying psi function to variance's sample
			...
			next1 = spot_variance.first + model->risk_free_rate() * delta_t * spot_variance.first + sqrt(model->local_volatility(_time_points[time_idx - 1],v,spot_variance.first)) * spot_variance.first * model->psi_function(spot_variance.second) * sqrt(delta_t) * z1;
		}
		...
		next2 = spot_variance.second + model->variance_drift(_time_points[time_idx-1], spot_variance.second) * delta_t + model->variance_diffusion(_time_points[time_idx-1], spot_variance.second) * sqrt(delta_t)*z3;
...
Pair PathSimulatorQE::next_step(const size_t& time_idx, const Pair& spot_variance, const vector<Pair>& spot_variance_sample) const
{
	...
	else
	{
		...
		if (time_idx == 1)
		{
			...
			double next_log_spot = current_log_spot + model->risk_free_rate()*delta_t - 0.5 * pow((model->get_dupire_vol()).local_volatility(_time_points[time_idx - 1], spot_variance.first), 2) / model->psi_function(spot_variance.second) * spot_variance.second * delta_t + (model->correlation() / model->vol_of_vol()) * r1 + rho1 * r2 * z3;
		...
		else
		{
			...
			double r1 = sqrt(model->local_volatility(_time_points[time_idx - 1], v, spot_variance.first)) * (next2 - model->kappa() * model->theta() * delta_t + spot_variance.second * c1);
			double r2 = sqrt(model->local_volatility(_time_points[time_idx - 1], v, spot_variance.first) * spot_variance.second * delta_t);
			double next_log_spot = current_log_spot + model->risk_free_rate() - 0.5 * model->local_volatility(_time_points[time_idx - 1], v, spot_variance.first) * spot_variance.second * delta_t + (model->correlation() / model->vol_of_vol()) * r1 + rho1 * r2 * z3;
			double next1 = exp(next_log_spot);
			return Pair(next1, next2);
		}
	}
