...
Pair PathSimulator::path_maturity()
{
	...
	for (size_t i = 1; i < _time_points.size(); i++)
	{
		...
		spot_variance = samples[rand() % samples.size()];
	}
...
Pair PathSimulatorEuler::next_step(const size_t& time_idx, const Pair& spot_variance, vector<Pair>& spot_variance_sample) const
{
	...
	next2 = spot_variance.second + model->variance_drift(_time_points[time_idx - 1], max(spot_variance.second,0)) * delta_t + model->variance_diffusion(_time_points[time_idx - 1], max(spot_variance.second, 0)) * sqrt(delta_t) * z3;
	// To avoid negative values of the variance (Full Truncation)
	next2=max(next2, 0);
	// Gives the asset discretization
	if (time_idx == 1)
	{
		next1 = spot_variance.first + model->risk_free_rate() * delta_t * spot_variance.first + sqrt(pow((model->get_dupire_vol()).local_volatility(_time_points[time_idx - 1], spot_variance.first), 2) / model->psi_function(max(spot_variance.second, 0))) * spot_variance.first * model->psi_function(max(spot_variance.second, 0)) * sqrt(delta_t) * z1;
	...
	next1 = spot_variance.first + model->risk_free_rate() * delta_t * spot_variance.first + sqrt(model->local_volatility(_time_points[time_idx - 1], spot_variance_sample, spot_variance.first)) * spot_variance.first * model->psi_function(max(spot_variance.second, 0)) * sqrt(delta_t) * z1;
	return Pair(next1, next2);
}
