...
Pair PathSimulatorEuler::next_step(const size_t& time_idx, const Pair& spot_variance, const vector<Pair>& spot_variance_sample) const
{
	...
	else
	{
		...
		else
		{
			...
			next1 = spot_variance.first + model->risk_free_rate() * delta_t * spot_variance.first + sqrt(model->local_volatility(_time_points[time_idx - 1], spot_variance, v, 2)) * spot_variance.first * model->psi_function(spot_variance.second) * sqrt(delta_t) * z1;
		}
