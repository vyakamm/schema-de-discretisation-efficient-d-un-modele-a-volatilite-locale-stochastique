...
Pair PathSimulator::path_maturity()
{
	...
	for (size_t i = 1; i < _time_points.size(); i++)
	{
		...
		for (k = 0; k < number_of_simulations && samples[k].second == 0 && i != _time_points.size() - 1; k++);
			spot_variance = samples[k]; //samples[rand() % samples.size()];
		...
		cout << "\n";
		cout << spot_variance.first <<" "<< spot_variance.second;
		cout << "\n";
	}
...
Pair PathSimulatorEuler::next_step(const size_t& time_idx, const Pair& spot_variance, vector<Pair>& spot_variance_sample) const
{
	...
	if (time_idx == 1)
	{
		next1 = spot_variance.first + model->risk_free_rate() * delta_t * spot_variance.first + sqrt(pow((model->get_dupire_vol()).local_volatility(_time_points[time_idx - 1], spot_variance.first), 2) / model->psi_function(max(spot_variance.second, 0.0))) * spot_variance.first * model->psi_function(max(spot_variance.second, 0.0)) * sqrt(delta_t) * z1;
